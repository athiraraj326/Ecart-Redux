      ...
      <div style={{paddingTop:'100px'}} className="flex content-center items-center mx-5">
        <div className="grid grid-cols-2 gap-5 items-center">
          ...
          <div>
            ...
            <h4 className="font-bold text-red-600 text-2xl my-3">$ {product?.price}</h4>
            <h4>Brand: {product?.brand}</h4>
            <h4 className='my-3'>Category: {product?.category}</h4>
            <p>
            ...
            <h3 className='font-bold my-5'>Client Review</h3>
            {
              ...
              product?.reviews?.map((item,index)=>(
                <div key={index} className='border rounded p-2 mb-2'>
                  <h5><span className='font-bold'>{item?.reviewerName} : </span> {item?.comment}</h5>
